...
module.exports = $ => {
  ...
  $.mongoose.multitenancy = {
    ...
    injectBaseProperties (scope, req, doc, isUpdate) {
      ...
      const getValue = (val, path, schema) => {
        if (!val) {
          return val;
        }
        ...
        const setSubValues = (sub, skipMulti) => {
          if (schema.instance === 'Array' && !skipMulti) {
            return sub.forEach(e => setSubValues(e, true));
          }

          const subSchema = schema.schema;

          Object.keys(subSchema.paths).forEach(subPath => {
            if (!sub[subPath]) {
              return;
            }

            const newValue = getValue(sub[subPath], subPath, subSchema.paths[subPath]);

            if (sub.setValue) {
              sub.setValue(subPath, newValue);
            }
            else {
              sub[subPath] = newValue;
            }
          });
        };
        ...
        if (schema.schema) {
          ...
          const id = $.utils.getId(val);
          if ($.utils.isObjectId(id)) {
            return $.utils.getObjectId(id);
          }
        }
      ...
      const historyFields = Object.keys(paths)
        .filter(path =>
          (paths[path].options && paths[path].options.history) ||
          (paths[path].caster && paths[path].caster.options && paths[path].caster.options.history))
        .filter(path => {
          const from = getValue((doc._original || {})[path], path);
          const to = getValue(doc[path], path);

          return $.utils.isObjectId(to) ? !$.utils.idEquals(from, to) : from !== to;
        });
      ...
      doc.history = [{
        ...
        from: historyFields.reduce((from, field) => Object.assign(from, {
          [field]: getValue((doc._original || {})[field], field)
        }), {}),
        to: historyFields.reduce((to, field) => Object.assign(to, {
          [field]: getValue(doc[field], field)
        }), {})
      }];
    },
