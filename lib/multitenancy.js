...
module.exports = $ => {
  ...
  $.mongoose.multitenancy = {
    ...
    injectBaseProperties (scope, req, doc, isUpdate) {
      ...
      const getValue = val => $.utils.isObjectId($.utils.getId(val)) ? $.utils.getId(val) : val;
      const historyFields = Object.keys(paths)
        ...
        .filter(path => {
          const from = getValue((doc._original || {})[path]);
          const to = getValue(doc[path]);
      ...
      doc.history = [Object.assign({
        from: historyFields.reduce((from, field) => Object.assign(from, {
          [field]: getValue((doc._original || {})[field])
        }), {}),
        to: historyFields.reduce((to, field) => Object.assign(to, {
          [field]: getValue(doc[field])
        }), {})
      }, doc.update_spec)];
    },
