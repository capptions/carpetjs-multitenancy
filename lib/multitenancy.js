...
module.exports = function ($) {
  ...
  var attachBaseProperties = function attachBaseProperties(req, isNew) {
    ...
    this.update_spec = { 'timestamp': Date.now() };

    if (req.appName && !this.app_name) {
      this.update_spec['app_name'] = req.appName;
    }

    if (req.userId) {
      this.update_spec['updated_by'] = req.userId;
    }

    if (req.organizationId) {
      this.update_spec['organization'] = req.organizationId;
    }
  };
  ...
  $.mongoose.Schema = function () {
    ...
    this.pre('save', function (next, req) {
      console.log(validRequest(req));
      if (validRequest(req)) {
        this._req = req;
        attachOrganizationId.call(this, req, this);
        attachBaseProperties.call(this, req, true);
      }
      next();
    });
