  ...
  $.mongoose.multitenancy = {
    ...
    injectBaseProperties (scope, req, doc, isUpdate) {
      ...
      const getValue = (val, path, schema) => {
        ...
        const setSubValues = sub => {
          if (schema.instance === 'Array') {
            return sub.forEach(setSubValues);
          }

          Object.keys(schema.schema.paths).forEach(subPath => {
            if (!sub[subPath]) {
              return;
            }

            const newValue = getValue(sub[subPath], subPath, schema.schema.paths[subPath]);
