...
module.exports = $ => {
  ...
  $.mongoose.multitenancy = {
    ...
    modelFromScope (scope) {
      if (scope.modelName) {
        ...
        return Object.keys($.mongoose.models).find(m => $.mongoose.models[m].collection === scope);
      }
    },
    ...
    injectBaseProperties (doc, req, update) {
  ...
  $.mongoose.Model.update = function (req, ...args) {
    ...
    $.mongoose.multitenancy.injectBaseProperties(doc, req, true);
  ...
  $.mongoose.Model.prototype.save = function (req, ...args) {
    ...
    $.mongoose.multitenancy.injectBaseProperties(this, req, !this.isNew);
  ...
  $.mongoose.Collection.prototype.insert = function (req, ...args) {
    ...
    if ($.mongoose.multitenancy.isRequest(req)) {
      args[0] = args[0].map(doc => {
        $.mongoose.multitenancy.injectProperties(self, req, doc, 'insert');
        $.mongoose.multitenancy.injectBaseProperties(doc, req);
        return doc;
      });
    }
